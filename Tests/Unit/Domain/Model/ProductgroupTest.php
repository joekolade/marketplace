...
/**
 * Test case for class \JS\Marketplace\Domain\Model\Productgroup.
 *
...
class ProductgroupTest extends \TYPO3\CMS\Core\Tests\UnitTestCase
{
	...
	{
		$this->subject = new \JS\Marketplace\Domain\Model\Productgroup();
	}
	 ...
	 * @test
	 */
	public function getProductsubgroupsReturnsInitialValueForProductsubgroup()
	{
		$newObjectStorage = new \TYPO3\CMS\Extbase\Persistence\ObjectStorage();
		$this->assertEquals(
			$newObjectStorage,
			$this->subject->getProductsubgroups()
		);
	}
	 ...
	 * @test
	 */
	public function setProductsubgroupsForObjectStorageContainingProductsubgroupSetsProductsubgroups()
	{
		$productsubgroup = new \JS\Marketplace\Domain\Model\Productsubgroup();
		$objectStorageHoldingExactlyOneProductsubgroups = new \TYPO3\CMS\Extbase\Persistence\ObjectStorage();
		$objectStorageHoldingExactlyOneProductsubgroups->attach($productsubgroup);
		$this->subject->setProductsubgroups($objectStorageHoldingExactlyOneProductsubgroups);

		$this->assertAttributeEquals(
			$objectStorageHoldingExactlyOneProductsubgroups,
			'productsubgroups',
			$this->subject
		);
	}
	 ...
	 * @test
	 */
	public function addProductsubgroupToObjectStorageHoldingProductsubgroups()
	{
		$productsubgroup = new \JS\Marketplace\Domain\Model\Productsubgroup();
		$productsubgroupsObjectStorageMock = $this->getMock('TYPO3\\CMS\\Extbase\\Persistence\\ObjectStorage', array('attach'), array(), '', FALSE);
		$productsubgroupsObjectStorageMock->expects($this->once())->method('attach')->with($this->equalTo($productsubgroup));
		$this->inject($this->subject, 'productsubgroups', $productsubgroupsObjectStorageMock);
	 ...
	 * @test
	 */
	public function removeProductsubgroupFromObjectStorageHoldingProductsubgroups()
	{
		$productsubgroup = new \JS\Marketplace\Domain\Model\Productsubgroup();
		$productsubgroupsObjectStorageMock = $this->getMock('TYPO3\\CMS\\Extbase\\Persistence\\ObjectStorage', array('detach'), array(), '', FALSE);
		$productsubgroupsObjectStorageMock->expects($this->once())->method('detach')->with($this->equalTo($productsubgroup));
		$this->inject($this->subject, 'productsubgroups', $productsubgroupsObjectStorageMock);
